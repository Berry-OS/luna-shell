...
".widget{position:absolute;background:linear-gradient(135deg,rgba(18,19,42,.52) 0%,rgba(10,11,26,.65) 100%);backdrop-filter:blur(24px) saturate(1.6);border:1px solid rgba(255,255,255,.12);border-radius:22px;box-shadow:0 10px 28px rgba(0,0,0,.22),inset 0 1px 0 rgba(255,255,255,.08);padding:16px 18px;}"
...
".wg_clock_icon{display:flex;align-items:center;justify-content:center;width:24px;height:24px;border-radius:8px;color:rgba(215,212,242,.75);font-size:12px;cursor:pointer;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.08);}"
".wg_clock_icon:hover{background:rgba(255,255,255,.14);color:#fff;}#wg_stopwatch_open{position:absolute;right:0;top:0;z-index:20;}"
...
".st_row{display:flex;flex-direction:row;justify-content:space-between;margin-top:8px;}.st_label{color:rgba(215,212,242,.75);font-size:11px;font-weight:500;}.st_val{color:#fff;font-size:11px;font-weight:700;text-align:right;}"
".st_bar{height:4px;border-radius:2px;background:rgba(255,255,255,.10);margin-top:5px;position:relative;overflow:hidden;}.st_fill{position:absolute;left:0;top:0;height:100%;width:20%;border-radius:2px;background:linear-gradient(90deg,#c9a6ff,#8d7bff);}.st_fill.g_mem{background:linear-gradient(90deg,#bf5af2,#9b3fe0);}.st_fill.g_disk{background:linear-gradient(90deg,#30d158,#25a244);}"
...
"#wg_wx_forecast{display:flex;flex-direction:row;gap:6px;justify-content:space-between;}.wf_day{flex:1 1 0;text-align:center;font-size:11px;color:rgba(255,255,255,.55);}.wf_name{font-size:10px;opacity:.7;margin-bottom:2px;}.wf_icon{font-size:16px;color:#c9a6ff;margin:4px 0;}.wf_temp{font-weight:600;font-size:11px;color:#fff;margin-top:2px;}";
...
static const char* luna_shell_rt_div_close(const char* open) {
    ...
    while (*p) {
        ...
        if (!strncmp(p, "<div", 4) && (p[4] == '>' || p[4] == ' ' || p[4] == '\t' || p[4] == '\n')) {
            depth++;
            p += 4;
            continue;
        }
...
#define LUNA_SHELL_RT_IS_CONST(x) LUNA_SHELL_RT_HAS_COMMA(LUNA_SHELL_RT_CAT(LUNA_SHELL_RT_IS_CONST_,x))
#define LUNA_SHELL_RT_IS_INT(x) LUNA_SHELL_RT_HAS_COMMA(LUNA_SHELL_RT_CAT(LUNA_SHELL_RT_IS_INT_,x))
#define LUNA_SHELL_RT_IS_VOID(x) LUNA_SHELL_RT_HAS_COMMA(LUNA_SHELL_RT_CAT(LUNA_SHELL_RT_IS_VOID_,x))
...
#define luna_parse_html(arg) LUNA_SHELL_RT_PARSE_HTML_SEL(LUNA_SHELL_RT_IS_CONST(arg))(arg)
...
#define luna_load_html_file(arg) LUNA_SHELL_RT_LOAD_HTML_SEL(LUNA_SHELL_RT_IS_CONST(arg))(arg)
