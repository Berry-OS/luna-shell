...
struct luna_gpu_plane { int fd; const void *pixels; uint32_t width,height,stride,offset,fourcc; int32_t x,y; };
...
int luna_gpu_render(struct luna_gpu *g,const uint32_t *pixels,uint32_t w,uint32_t h,struct luna_gpu_output *out) {
  ...
  glBindBuffer(GL_ARRAY_BUFFER,g->vbo); glBufferData(GL_ARRAY_BUFFER,sizeof(q),q,GL_STREAM_DRAW);
...
int luna_gpu_render_planes(struct luna_gpu *g,const struct luna_gpu_plane *p,uint32_t count,struct luna_gpu_output *out) {
  ...
  for(uint32_t i=0;i<count;i++){
    EGLImageKHR im=EGL_NO_IMAGE_KHR; GLuint tex;glGenTextures(1,&tex);glBindTexture(GL_TEXTURE_2D,tex);
    if(p[i].fd>=0){ EGLint a[]={EGL_WIDTH,(EGLint)p[i].width,EGL_HEIGHT,(EGLint)p[i].height,
      EGL_LINUX_DRM_FOURCC_EXT,(EGLint)p[i].fourcc,EGL_DMA_BUF_PLANE0_FD_EXT,p[i].fd,
      EGL_DMA_BUF_PLANE0_OFFSET_EXT,(EGLint)p[i].offset,EGL_DMA_BUF_PLANE0_PITCH_EXT,(EGLint)p[i].stride,EGL_NONE};
      im=g->create_image(g->display,EGL_NO_CONTEXT,EGL_LINUX_DMA_BUF_EXT,NULL,a);
      if(im==EGL_NO_IMAGE_KHR){glDeleteTextures(1,&tex);return 0;} g->image_texture(GL_TEXTURE_2D,im);
    }else glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,p[i].width,p[i].height,0,GL_BGRA_EXT,GL_UNSIGNED_BYTE,p[i].pixels);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    ...
    glDrawArrays(GL_TRIANGLE_STRIP,0,4);glDeleteTextures(1,&tex);if(im!=EGL_NO_IMAGE_KHR)g->destroy_image(g->display,im);
  }
  if(!eglSwapBuffers(g->display,g->egl_surface)) return 0;
...
void luna_gpu_destroy(struct luna_gpu *g){if(!g)return;if(g->pending)gbm_surface_release_buffer(g->surface,g->pending);if(g->front)gbm_surface_release_buffer(g->surface,g->front);eglMakeCurrent(g->display,EGL_NO_SURFACE,EGL_NO_SURFACE,EGL_NO_CONTEXT);if(g->egl_surface)eglDestroySurface(g->display,g->egl_surface);if(g->context)eglDestroyContext(g->display,g->context);if(g->display)eglTerminate(g->display);if(g->surface)gbm_surface_destroy(g->surface);if(g->dev)gbm_device_destroy(g->dev);free(g);}
