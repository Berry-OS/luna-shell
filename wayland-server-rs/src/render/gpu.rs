...
#[repr(C)]
struct Plane { fd: c_int, pixels:*const u8, dev:u64, ino:u64, serial:u64, width:u32, height:u32, stride:u32, offset:u32, fourcc:u32, x:i32, y:i32 }
...
impl GpuComposer {
    pub fn new(fd: RawFd, width: u32, height: u32) -> Option<Self> {
        ...
        (!p.is_null()).then_some(Self { ctx: p, planes: Vec::new() })
    }
    ...
    pub fn render_dmabufs(&mut self, surfaces:&[(i32,i32,crate::shm::ShmBuffer)], bitmap:Option<super::GpuBitmap<'_>>) -> Option<(*mut c_void,u32,u32)> {
        ...
        for (x, y, b) in surfaces {
            ...
            self.planes.push(Plane{fd,pixels:if fd<0{b.upload_ptr()?}else{std::ptr::null()},dev,ino,serial:b.serial(),width,height:b.height.try_into().ok()?,stride,offset:b.offset.try_into().ok()?,fourcc:if b.format==crate::shm::FORMAT_XRGB8888{0x34325258}else{0x34325241},x:*x,y:*y});
        }
        if let Some(b)=bitmap { self.planes.push(Plane{fd:-1,pixels:b.pixels.as_ptr() as *const u8,dev:0,ino:0,serial:0,width:b.width,height:b.height,stride:b.width*4,offset:0,fourcc:0x34325241,x:b.x,y:b.y}); }
        let mut out=Output::default();
