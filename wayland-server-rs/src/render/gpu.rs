...
struct DmabufFormat { fourcc: u32, _pad: u32, modifier: u64 }
#[repr(C)]
struct Plane { fd: c_int, pixels:*const u8, dev:u64, ino:u64, serial:u64, modifier:u64, width:u32, height:u32, stride:u32, offset:u32, fourcc:u32, x:i32, y:i32 }
...
impl GpuComposer {
    pub fn new(fd: RawFd, width: u32, height: u32) -> Option<Self> {
        ...
        let written = unsafe {
            luna_gpu_query_dmabuf_formats(p, queried.as_mut_ptr(), MAX_FORMAT_MODIFIERS)
        }.min(MAX_FORMAT_MODIFIERS) as usize;
        let mut dmabuf_formats: Vec<(u32, u64)> = queried[..written]
            .iter().map(|f| (f.fourcc, f.modifier)).collect();
        dmabuf_formats.sort_unstable();
        dmabuf_formats.dedup();
        Some(Self { ctx: p, planes: Vec::new(), dmabuf_formats })
    }
    ...
    fn push_plane(&mut self, x: i32, y: i32, b: &crate::shm::ShmBuffer) -> Option<()> {
        ...
        self.planes.push(Plane{fd,pixels:if fd<0{b.upload_ptr()?}else{std::ptr::null()},dev,ino,serial:b.serial(),modifier:b.dmabuf_modifier().unwrap_or(0),width,height:b.height.try_into().ok()?,stride,offset:b.offset.try_into().ok()?,fourcc:if b.format==crate::shm::FORMAT_XRGB8888{0x34325258}else{0x34325241},x,y});
        Some(())
    }

    pub fn render_dmabufs(
        &mut self,
        surfaces:&[(i32,i32,crate::shm::ShmBuffer)],
        extra_surface:Option<(i32,i32,&crate::shm::ShmBuffer)>,
        ...
        if let Some((x,y,b))=extra_surface { self.push_plane(x,y,b)?; }
        if let Some(b)=bitmap { self.planes.push(Plane{fd:-1,pixels:b.pixels.as_ptr() as *const u8,dev:0,ino:0,serial:0,modifier:0,width:b.width,height:b.height,stride:b.width*4,offset:0,fourcc:0x34325241,x:b.x,y:b.y}); }
        let mut out=Output::default();
